...
export class UniversalFormSheetComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_BOTTOM_SHEET_DATA) public data: { data: { entityType: EGoodsTabs, item?: any } },
    private bottomSheetRef: MatBottomSheetRef<UniversalFormSheetComponent>,
    ...
    this.notifications
      ...
      if (result === 'remove') {
        this.selectRemove(this.data.data.item).subscribe((response) => {
          if (response) {
            this.bottomSheetRef.dismiss({response, data: this.data.data.item});
          } else {
